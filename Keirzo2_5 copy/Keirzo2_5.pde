...
int hdrumY;
int hdrumX;
float bubbleMeanAccelX;
float bubbleMeanAccelY;
...
void setup() {
  ...
  for (int i = 0; i<bubbles.length; i++) {
    bubbles[i] = new BubbleSystem(random(0.1, 5));
  }
  ...
  oscP5.plug(this, "MeanAccelX", "/meanAccelX");
  oscP5.plug(this, "MeanAccelX", "/meanAccelX");
}
...
//float targetxt2, targetyt2, targety2t2;
  ...
  for (int i = 0; i < bubbles.length; i++) {
   ...
   float slider = map(sliderPot, 0, 1023, 0., 0.01);
    
    wind = new PVector(slider, slider);
    gravity = new PVector(slider, slider);


    bubbles[i].applyForce(wind);
    bubbles[i].applyForce(gravity);
    bubbles[i].update(posx, posy);
    bubbles[i].display();
  ...
  for (Bubble b : mbubbles) {
    wind = new PVector(posx*0.1, posy*0.1);

    gravity = new PVector(bubbleMeanAccelX, bubbleMeanAccelY);
    b.checkEdges();
    b.update();
    b.display();
    b.applyForce(wind);
...
public void MeanAccelX(float meanAccelX) {

  bubbleMeanAccelX = meanAccelX;
}

public void MeanAccelY(float meanAccelY) {

  bubbleMeanAccelY = meanAccelY;
}
